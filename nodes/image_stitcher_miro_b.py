...
from teach_repeat import image_processing_miro_b
...
class image_stitcher:
	...
	def setup_parameters(self):
		...
		if self.left_cal_file is not None:
			with open(self.left_cal_file,'r') as f:
				self.cam_left_calibration = image_processing_miro_b.yaml_to_camera_info(yaml.load(f.read(), Loader=yaml.SafeLoader))
		else:
		...
		if self.right_cal_file is not None:
			with open(self.right_cal_file,'r') as f:
				self.cam_right_calibration = image_processing_miro_b.yaml_to_camera_info(yaml.load(f.read(), Loader=yaml.SafeLoader))
		else:
	...
	def process_image_data(self, msg):
		...
		full_image = image_processing_miro_b.stitch_stereo_image_message(msg.left, msg.right, compressed=True)

		image_msg = image_processing_miro_b.image_to_msg(full_image, 'mono8')
